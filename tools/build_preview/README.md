...
- **Some `ui/p2` backgrounds are still shown against the wrong tile-sheet.** `--dump-ui` renders screen × tile-sheet × palette combinations because it *cannot know* which belong together, and only the game knows. `convert_bg_dir` now keeps a spread across tile-sheets per screen (preferring the palette whose index matches the sheet, which is the pairing shape `ov006`/`ov008` use for `res.p2`), so the correct pairing is among the kept variants rather than hidden — before, it kept four palettes of tile-sheet 0 and nothing else, which made whole packs like `res.p2` look striped. Wrong pairings still sit beside the right one; the gallery cannot drop them without the real table. `ttl.p2` has one (`data_ov000_0205a9d4`, an 8×3 screen→(tile,palette) table), but `res.p2` does **not** — `ov006`/`ov008` bind tile+palette explicitly and never bind a screen from the pack (NSCR arg always -1), so there is nothing to read. Sprite cells and 3D textures are unaffected either way.
